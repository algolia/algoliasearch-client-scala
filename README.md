...
[Algolia's website](https://www.algolia.com/doc/api-client/scala/).
...
1. [Search multiple indices - `multiQueries`](#search-multiple-indices---multiqueries)
1. [Get Objects - `get from "index"`](#get-objects---get-from-index)
1. [Search for facet values - `search into facet values`](#search-for-facet-values---search-into-facet-values)

**Indexing**

1. [Add Objects - `index into`](#add-objects---index-into)
1. [Update objects - `index int`](#update-objects---index-int)
1. [Partial update objects - `update attribute`](#partial-update-objects---update-attribute)
1. [Delete objects - `delete from`](#delete-objects---delete-from)
1. [Delete by query - `deleteByQuery`](#delete-by-query---deletebyquery)
1. [Wait for operations - `waitFor task`](#wait-for-operations---waitfor-task)

**Settings**

1. [Get settings - `settings`](#get-settings---settings)
1. [Set settings - `changeSettings`](#set-settings---changesettings)
...
1. [Move index - `move index`](#move-index---move-index)
...
1. [Save synonym - `save synonym`](#save-synonym---save-synonym)
1. [Batch synonyms - `save synonyms`](#batch-synonyms---save-synonyms)
1. [Editing Synonyms](#editing-synonyms)
1. [Delete synonym - `delete synonym`](#delete-synonym---delete-synonym)
1. [Clear all synonyms - `clear synonyms of index`](#clear-all-synonyms---clear-synonyms-of-index)
1. [Get synonym - `get synonym`](#get-synonym---get-synonym)
1. [Search synonyms - `search synonyms of`](#search-synonyms---search-synonyms-of)

**Advanced**

1. [Custom batch - `batch`](#custom-batch---batch)
1. [Backup / Export an index - `browse index`](#backup--export-an-index---browse-index)
1. [List api keys - `get allKeys`](#list-api-keys---get-allkeys)
1. [Add user key - `add key`](#add-user-key---add-key)
1. [Update user key - `update key`](#update-user-key---update-key)
1. [Delete user key - `delete key`](#delete-user-key---delete-key)
1. [Get key permissions - `get key`](#get-key-permissions---get-key)
1. [Get latest logs - `logs`](#get-latest-logs---logs)
1. [REST API](#rest-api)
...
## Search multiple indices - `multiQueries` 
...
## Search for facet values - `search into facet values` 
...
```scala
client.execute {
  search into "myIndex" facet "category" facetQuery "phone" query Query(
...
You can list all your indices along with their associated information (number of entries, disk size, etc.) with the `list.indices()` method:
...
**Note:** The move index method overrides the destination index, and deletes the temporary one.
  In other words, there is no need to call the `clear index` or `delete index` methods to clean the temporary index.
...
val publicKey = client.generateSecuredApiKey("YourSearchOnlyApiKey", Query(validUntil = Some(validUntil))
```
...
val publicKey = client.generateSecuredApiKey("YourSearchOnlyApiKey", Query(restrictIndices = Some(Seq("index1", "index2")))
```
